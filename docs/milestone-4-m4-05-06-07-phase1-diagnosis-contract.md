...
**Status:** Implemented, awaiting Solution Architect review. Not applied to `baslon_os`. No Baslon Digital diagnosis has run.
...
9. **B-19:** unchanged.
10. **Rollout:** `0007` is applied to `baslon_os_test` and PGlite only. **`baslon_os` needs explicit Product Owner approval.**
